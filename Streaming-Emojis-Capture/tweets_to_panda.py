...
def fill_emojis_list(list,emojis_json):
	...
	print("List of {} emojis".format(len(list)))				
...
def fill_sample_including_emojis(data_attributes,file,emojis_list):
	for line in file:
		try:
			...
			# Take only tweets that have country specified
			if(tweet['place']['country'] != None):
				...
				if any(emoji in tweet['text'] for emoji in emojis_list):
					...
					data_attributes['est_age'].append(None)
					data_attributes['est_gender'].append(str(None))
